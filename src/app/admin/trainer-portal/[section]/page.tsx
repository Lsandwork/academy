...
export default async function AdminTrainerSectionPage({ params }: { params: Promise<{ section: string }> }) {
  const { section: sectionId } = await params;
  const section = getTrainerPortalSection(sectionId);
  if (!section) notFound();

  return <TrainerPortalHub section={section} basePath="/admin/trainer-portal" />;
}
