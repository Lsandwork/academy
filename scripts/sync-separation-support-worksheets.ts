...
const zipPath =
  process.argv[2] || path.join(process.env.HOME || "", "Downloads/fitdog_separation_support_worksheets_FIXED_LOGO.zip");
const destDir = path.join(process.cwd(), "content/worksheets/separation-support");
const bundleFolder = "fitdog_separation_support_worksheets_FIXED_LOGO";
const trackId = "separation-support";
...
const tmp = path.join(process.cwd(), ".tmp-separation-support-worksheets-sync");
...
fs.rmSync(tmp, { recursive: true, force: true });
console.log(`Synced ${synced} Separation Support worksheets to ${destDir}`);
