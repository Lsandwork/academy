...
const zipPath =
  process.argv[2] ||
  path.join(process.env.HOME || "", "Downloads/fitdog_fitdog_enrichment_at_home_worksheets_FIXED_LOGO.zip");
const destDir = path.join(process.cwd(), "content/worksheets/fitdog-enrichment-at-home");
const bundleFolder = "fitdog_fitdog_enrichment_at_home_worksheets_FIXED_LOGO";
...
const tmp = path.join(process.cwd(), ".tmp-enrichment-worksheets-sync");
...
fs.rmSync(tmp, { recursive: true, force: true });
console.log(`Synced ${synced} Fitdog Enrichment at Home worksheets to ${destDir}`);
