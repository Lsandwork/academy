...
const zipPath =
  process.argv[2] ||
  path.join(process.env.HOME || "", "Downloads/fitdog_leash_reactivity_reset_worksheets_FIXED_LOGO.zip");
const destDir = path.join(process.cwd(), "content/worksheets/leash-reactivity-reset");
const bundleFolder = "fitdog_leash_reactivity_reset_worksheets_FIXED_LOGO";
...
const tmp = path.join(process.cwd(), ".tmp-leash-reactivity-worksheets-sync");
...
fs.rmSync(tmp, { recursive: true, force: true });
console.log(`Synced ${synced} Leash Reactivity Reset worksheets to ${destDir}`);
